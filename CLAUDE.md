...
- **`get_ollama_free_models()`** - Cached (`@st.cache_data(ttl="1d")`) unauthenticated GET of the published list at `https://letsgada.github.io/ollama-cloud-free-models-list/models.json` (refreshed daily). Falls back to a hardcoded list (`gpt-oss:120b`, `glm-4.7`, `gpt-oss:20b`) on network failure or empty result. Results are sorted by `_OLLAMA_PREFERENCE_ORDER` (curated for transcript-summarization fit), with unranked / newly added models alphabetical at the end. Used to populate the Ollama model selectbox in the sidebar.
